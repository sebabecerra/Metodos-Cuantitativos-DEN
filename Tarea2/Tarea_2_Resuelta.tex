...
En las ecuaciones, $X_c$ representa la presión competitiva centrada en su media, $W_c$ representa la tolerancia al fracaso centrada en su media, $M$ corresponde a la conducta innovadora del empleado y $Y$ al desempeño innovador de la empresa. Los coeficientes $a_1$, $a_2$, $a_3$, $b$, $c'$ y $c$ representan los efectos estimados en cada camino del modelo: $a_1$ es la asociación entre presión competitiva y conducta innovadora cuando la tolerancia está en su media, $a_2$ es la asociación entre tolerancia y conducta innovadora cuando la presión está en su media, $a_3$ es la interacción, $b$ es la asociación entre conducta innovadora y desempeño, $c'$ es el efecto directo residual y $c$ es el efecto total.
...
El coeficiente $a_1$ representa el efecto de la presión competitiva cuando la tolerancia al fracaso está en su media, porque $W_c=0$. El coeficiente $a_2$ representa el efecto de la tolerancia al fracaso cuando la presión competitiva está en su media, porque $X_c=0$. Finalmente, $a_3$ indica cuánto cambia el efecto de $X_c$ sobre $M$ por cada unidad adicional de $W_c$. Si $a_3$ es significativo, existe moderación.
...
con IC95\% bootstrap $[0.3758,0.5184]$. Como el intervalo no contiene cero, el efecto indirecto varía significativamente con la tolerancia al fracaso. Este resultado complementa H4 y H5, mostrando que el mecanismo indirecto es más fuerte cuando aumenta la tolerancia al fracaso.
...
\begin{table}[H]
\centering
\caption{Resumen final de hipótesis de la Pregunta 1}
\label{tab:p1-resumen-hipotesis}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llll}
\toprule
Hipótesis & Relación evaluada & Evidencia principal & Decisión \\
\midrule
H1 & Presión competitiva $\rightarrow$ conducta innovadora & $a_1=2.0008$, $p<.001$ & Apoyada \\
H2 & Conducta innovadora $\rightarrow$ desempeño innovador & $b=0.8577$, $p<.001$ & Apoyada \\
H3 & Efecto directo presión competitiva $\rightarrow$ desempeño & $c'=0.0265$, $p=.590$ & No apoyada \\
H4 & Efecto indirecto vía conducta innovadora & IC95\% bootstrap sin cero & Apoyada \\
H5 & Moderación de tolerancia al fracaso sobre $X\rightarrow M$ & $a_3=0.5248$, $p<.001$ & Apoyada \\
\bottomrule
\end{tabular}}
\end{table}

En conjunto, los resultados apoyan H1, H2, H4 y H5. El efecto directo planteado en H3 no recibe apoyo empírico, por lo que la evidencia es consistente con una mediación \textit{indirect-only}.
...
La decisión se basó en autovalores, gráfico de sedimentación y análisis paralelo con 1.000 matrices aleatorias. El gráfico de sedimentación muestra los autovalores ordenados de mayor a menor y permite observar el punto donde la curva comienza a aplanarse; desde ese punto, los factores adicionales aportan poca información nueva. El criterio de autovalor mayor que uno se consideró solo como referencia, porque puede sugerir más factores de los realmente respaldados por los datos.

Los primeros cuatro autovalores observados superaron el percentil 95 de los autovalores aleatorios. La decisión del cuarto factor fue ajustada: 1.505 frente a un umbral de 1.501. Este valor corresponde al percentil 95 del cuarto autovalor obtenido en las 1.000 matrices aleatorias generadas para el análisis paralelo. Por ello se verificó con la media de los autovalores aleatorios, donde el cuarto factor también fue retenido y el quinto quedó claramente por debajo. Por esta razón, el análisis paralelo suele considerarse uno de los criterios más confiables para determinar el número de factores. La decisión no se basa solo en la diferencia numérica, sino también en la teoría previa y en la interpretabilidad de la solución de cuatro factores.
...
Una vez retenidos cuatro factores, se estimó la solución factorial principal mediante máxima verosimilitud y rotación Oblimin. La elección de máxima verosimilitud se justifica porque la pregunta busca evaluar dimensiones latentes de la teoría de Blend, no solamente reducir los ítems a un conjunto de componentes. En este enfoque, el modelo intenta explicar la varianza común entre los ítems mediante factores latentes y separarla de la varianza específica o de error.

Para transparentar la decisión metodológica, se estimó también la misma solución de cuatro factores con el método principal, manteniendo la misma muestra imputada y la misma rotación Oblimin. El método principal reporta una varianza acumulada mayor, porque reproduce una proporción más alta de la varianza observada de los ítems. Sin embargo, explicar más varianza observada no implica necesariamente representar mejor constructos latentes. Por esta razón, máxima verosimilitud se mantiene como especificación principal, mientras que el método principal se presenta como análisis de sensibilidad.

\begin{table}[H]
\centering
\caption{Comparación de métodos de extracción con la misma muestra imputada}
\begin{tabular}{lccc}
\toprule
Método & Rotación & Observaciones & Varianza acumulada 4 factores \\
\midrule
Máxima verosimilitud & Oblimin & 239 & 39.05\% \\
Principal & Oblimin & 239 & 47.82\% \\
\bottomrule
\end{tabular}
\end{table}

La rotación Oblimin se utilizó porque es una rotación oblicua: permite que los factores se correlacionen entre sí. Esta decisión es coherente con el enunciado, que indica que las cuatro características pueden estar correlacionadas, y con los resultados empíricos, donde las correlaciones entre factores alcanzaron un máximo de 0.471. Por tanto, no se fuerza artificialmente independencia entre dimensiones.

La solución principal con máxima verosimilitud explica 39.05\% de la varianza acumulada. En una rotación oblicua este porcentaje debe interpretarse con cautela, porque los factores pueden correlacionarse y la varianza no se reparte de forma completamente independiente entre dimensiones. Se interpreta la matriz patrón porque representa la contribución específica de cada factor controlando las correlaciones entre ellos. Se consideraron relevantes las cargas factoriales absolutas cercanas o superiores a 0.40. Un ítem se consideró complejo cuando además presentaba una carga secundaria cercana o superior a 0.30, o cuando la diferencia entre sus dos cargas principales era pequeña.

\begin{table}[H]
\centering
\caption{Varianza explicada por la solución principal de cuatro factores}
...
\subsection{Apéndice de sensibilidad: especificaciones alternativas sin ID}

Como verificación adicional, se estimaron dos especificaciones alternativas usando solamente los 28 ítems del cuestionario, sin incluir la variable administrativa \texttt{ID}. La primera usa el método principal con eliminación listwise y rotación Oblimin; la segunda usa residuos mínimos con eliminación listwise y rotación Varimax.

\begin{table}[H]
\centering
\small
\caption{Especificaciones alternativas de sensibilidad sin ID}
\begin{tabular}{p{0.43\textwidth}cc}
\toprule
Criterio & Principal + Oblimin & Minres + Varimax \\
\midrule
Variables incluidas & 28 ítems & 28 ítems \\
Variable \texttt{ID} incluida & No & No \\
Tratamiento de faltantes & Eliminación listwise & Eliminación listwise \\
Observaciones usadas & 231 & 231 \\
Método de extracción & Principal & Minres \\
Rotación & Oblimin & Varimax \\
Varianza acumulada de la solución de 4 factores & 47.88\% & 46.23\% \\
\bottomrule
\end{tabular}
\end{table}

La alternativa con método principal y eliminación listwise alcanza 47.88\%, muy cerca del 47.82\% obtenido con método principal e imputación por mediana. Por tanto, la diferencia frente al 39.05\% del análisis principal se debe principalmente al método de extracción y no al tratamiento de los valores faltantes. La alternativa Varimax también produce una solución interpretable, pero no se adopta como referencia porque fuerza factores no correlacionados.

La variable \texttt{ID} se excluye en todos los análisis porque no es un ítem del EECS-R, no mide ningún rasgo latente y su inclusión alteraría artificialmente KMO, Bartlett, autovalores y cargas factoriales.
...
Los análisis se realizaron con \texttt{lavaan} en R. Primero se estimó un análisis factorial confirmatorio (CFA, por sus siglas en inglés) para evaluar si los ítems representan adecuadamente sus constructos. Luego se estimaron modelos de ecuaciones estructurales (SEM) para analizar simultáneamente las relaciones entre constructos latentes, considerando el error de medición. La estimación principal se realizó con máxima verosimilitud robusta (MLR) y máxima verosimilitud con información completa (FIML). MLR entrega errores estándar y pruebas robustas frente a desviaciones de normalidad; FIML permite utilizar toda la información disponible. En esta base no hubo valores perdidos en las variables de la Pregunta 3.
...
El CFA de cinco factores correlacionados presentó un ajuste muy bueno. El ajuste se evaluó mediante el índice de ajuste comparativo (CFI), el índice Tucker--Lewis (TLI), el error cuadrático medio de aproximación (RMSEA) y el residuo cuadrático medio estandarizado (SRMR). Los índices robustos estimados con \texttt{lavaan} fueron:
...
La confiabilidad se evaluó mediante alfa de Cronbach y confiabilidad compuesta (CR, \textit{composite reliability}). La validez convergente se evaluó mediante varianza media extraída (AVE, \textit{average variance extracted}):
...
La correlación latente más alta fue $r_{PA,IP}=.562$, inferior a la menor raíz de AVE, correspondiente a ST ($\sqrt{AVE}=.720$). Por tanto, se cumple el criterio de Fornell--Larcker. Además, se calculó la razón heterotrait--monotrait (HTMT), un criterio adicional de validez discriminante. HTMT compara las correlaciones entre constructos distintos con las correlaciones dentro de un mismo constructo; el HTMT máximo fue .569, claramente inferior al umbral conservador de .85.
...
\newpage
\section*{Anexo 1: Código y output pregunta 1}
\addcontentsline{toc}{section}{Anexo 1: Código y output pregunta 1}
\begin{center}
\vspace*{0.35\textheight}
{\Large \textbf{Anexo 1: Código y output pregunta 1}}\\[1em]
{\large Notebook ejecutado de la Pregunta 1}
\end{center}
\newpage
\includepdf[pages=-,pagecommand={}]{notebook_pdfs/Tarea_2_P1_Mediacion_Moderada.pdf}

\newpage
\section*{Anexo 2: Código y output pregunta 2}
\addcontentsline{toc}{section}{Anexo 2: Código y output pregunta 2}
\begin{center}
\vspace*{0.35\textheight}
{\Large \textbf{Anexo 2: Código y output pregunta 2}}\\[1em]
{\large Notebook ejecutado de la Pregunta 2}
\end{center}
\newpage
\includepdf[pages=-,pagecommand={}]{notebook_pdfs/Tarea_2_P2_Analisis_Factorial.pdf}

\newpage
\section*{Anexo 3: Código y output pregunta 3}
\addcontentsline{toc}{section}{Anexo 3: Código y output pregunta 3}
\begin{center}
\vspace*{0.35\textheight}
{\Large \textbf{Anexo 3: Código y output pregunta 3}}\\[1em]
{\large Notebook ejecutado de la Pregunta 3}
\end{center}
\newpage
\includepdf[pages=-,pagecommand={}]{notebook_pdfs/Tarea_2_P3_SEM.pdf}

\newpage
\section*{Anexo 4: Código y output pregunta 4}
\addcontentsline{toc}{section}{Anexo 4: Código y output pregunta 4}
\begin{center}
\vspace*{0.35\textheight}
{\Large \textbf{Anexo 4: Código y output pregunta 4}}\\[1em]
{\large Notebook ejecutado de la Pregunta 4}
\end{center}
\newpage
\includepdf[pages=-,pagecommand={}]{notebook_pdfs/Tarea_2_P4_MANOVA.pdf}
