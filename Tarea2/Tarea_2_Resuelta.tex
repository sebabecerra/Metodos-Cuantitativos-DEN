...
Los notebooks que acompañan este informe contienen el código completo, las salidas de cada procedimiento y la lógica utilizada. La primera pregunta reproduce el modelo 7 de PROCESS; la segunda aplica análisis factorial exploratorio; la tercera utiliza análisis factorial confirmatorio y ecuaciones estructurales; y la cuarta emplea MANOVA factorial.
...
Combinar ambas ideas produce una mediación moderada de primera etapa: el mecanismo indirecto $X \rightarrow M \rightarrow Y$ existe, pero su magnitud varía con $W$. Esta ubicación del moderador corresponde al modelo 7 de PROCESS y no al modelo 1, que solo estudiaría moderación, ni al modelo 4, que solo estudiaría mediación.
...
\end{align}
Las ecuaciones (1) y (2) constituyen el sistema estructural del modelo 7 de PROCESS; la ecuación (3) es la que permite evaluar H1. A partir de (1) y (2) se deriva el efecto indirecto condicional,
\begin{equation}
IE(W) = (a_1 + a_3 W)\, b,
\end{equation}
...
El modelo de mediación moderada se estima con las tres ecuaciones del planteamiento: la del mediador y la del resultado, que forman el sistema estructural, más la ecuación auxiliar del efecto total, que no modifica el modelo sino que permite reportar el efecto total requerido para evaluar H1 e interpretar la mediación. La expresión ``se estima el modelo simultáneamente'' significa que PROCESS ejecuta y reporta el sistema completo en una sola llamada; no significa que $M$ y $Y$ formen una única variable dependiente. Internamente se estima una regresión para $M$ y otra para $Y$, y luego se combinan sus coeficientes para formar los efectos indirectos condicionales.

Conviene precisar la relación entre las tres ecuaciones. En el modelo 4 de mediación simple se cumple exactamente la descomposición $c = c' + ab$. En el modelo 7 esa identidad no se cumple con $a_1$: aquí $c = 2.0281$ mientras que $c' + a_1 b = 0.0265 + (2.0008)(0.8577) = 1.7427$. La razón es que $a_1$ es el efecto de $X$ sobre $M$ condicional a $W$ y a la interacción, mientras que la ecuación del efecto total omite a $W$. La identidad OLS que sí se cumple exactamente es $c = c' + b\,\tilde a_1$, donde $\tilde a_1 = 2.3336$ proviene de la regresión simple $M \sim X$; en efecto, $0.0265 + (0.8577)(2.3336) = 2.0281$. La brecha entre $\tilde a_1$ y $a_1$ se explica por la correlación positiva entre presión y tolerancia ($r=0.174$). Además, el efecto total implicado por el sistema es condicional al moderador, $c(W) = c' + (a_1 + a_3 W)b$, igual a 1.7427 en el nivel medio de $W$. Por todo esto, $c$ se utiliza únicamente para documentar H1 como asociación total, y la evidencia formal de mediación descansa en los efectos indirectos condicionales y su bootstrap, no en la comparación entre $c$ y $c'$.

\paragraph{Pasos seguidos.} Se analizaron 200 observaciones sin datos perdidos. $X$ y $W$ se centraron en sus medias y se construyó el producto $X_cW_c$. El centrado facilita la interpretación de los efectos principales y reduce la colinealidad no esencial, pero no altera el ajuste ni el contraste de la interacción. Las regresiones se estimaron por mínimos cuadrados ordinarios con errores estándar robustos HC3. El notebook reproduce el procedimiento de PROCESS y añade un bootstrap de 5.000 remuestras, el estándar de PROCESS, para los efectos indirectos.
...
El modelo parcial presenta mejor CFI, TLI y RMSEA, y la prueba de diferencia entre modelos anidados es claramente significativa:
\[
...
Se rechazan las tres hipótesis nulas. Tanto distribución como tamaño influyen en la combinación de variables de lealtad y, además, interactúan: no son antecedentes independientes. Wilks $\Lambda$ representa la proporción de variación multivariada no explicada por el efecto. La interacción significativa es la evidencia formal de que la ventaja de un canal no es constante en ambos tamaños.
...
Los cuatro análisis requieren técnicas diferentes pero complementarias. La pregunta 1 confirma un mecanismo de mediación moderada: la tolerancia al fracaso amplifica la conversión de presión competitiva en innovación, con las seis hipótesis apoyadas. La pregunta 2 identifica cuatro factores compatibles, aunque no perfectamente, con la teoría de Blend, y la retención de cuatro factores es robusta a dos criterios de análisis paralelo. La pregunta 3 valida el modelo de medición mediante un CFA con ajuste excelente y favorece un modelo de mediación parcial según ajuste, prueba de diferencia y criterios de información, con diferencias estructurales por género. Finalmente, la pregunta 4 muestra que canal y tamaño afectan conjuntamente la lealtad y presentan interacción.
